...
function appointmentsIntersect(
  a: Readonly<Appointment>,
  b: Readonly<Appointment>,
): boolean {
  return (
    a.start.getTime() < b.start.getTime() + b.length * 60 * 1000 &&
...
export class DayComponent {
  ...
  readonly #dateParam$ = this.#route.params.pipe(
    map((params) => params["date"]),
  );
  readonly #date$ = this.#dateParam$.pipe(
    map((date) => {
  ...
  readonly appointments$ = combineLatest([
  ...
  readonly trackById = <T extends { id: PropertyKey }>(
    index: number,
    value: T,
  ) => value.id;

  readonly #offsets$: Observable<
    Map<
      Appointment,
      {
        width: string;
  ...
  > = this.appointments$.pipe(
    map((appointments) => {
      const clusters = appointments.reduce((clusters, appointment) => {
        if (
          (clusters[clusters.length - 1] ?? []).some((previous) =>
            appointmentsIntersect(previous, appointment),
          )
        ) {
        ...
        clusters.flatMap((cluster) => {
          return cluster.map((appointment, i) => {
            return [
              appointment,
              {
                width: 98 / cluster.length + "%",
                left: (98 / cluster.length) * i + "%",
              },
            ];
          });
        }),
  ...
  editAppointment(appointment: Readonly<Appointment>): void {
    AppointmentFormComponent.open(this.#matDialog, appointment).subscribe(
      (appointment) => {
        if (appointment) {
  ...
  addAppointment(
    minute: number = new Date().getHours() * 60 + new Date().getMinutes(),
  ): void {
    minute = Math.floor(minute / 15) * 15;
    AppointmentFormComponent.open(this.#matDialog, {
      id: -1,
      start: new Date(
