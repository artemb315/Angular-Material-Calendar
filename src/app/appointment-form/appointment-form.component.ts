...
export type AppointmentFormDialogResult = Appointment | null;
...
export class AppointmentFormComponent {
  ...
  readonly form = this.#fb.group({
    ...
    startDate: this.#fb.control(this.#data.start, [Validators.required]),
    startTime: this.#fb.control(
      this.#data.start.getHours() * 60 +
        Math.floor(this.#data.start.getMinutes() / 15) * 15,
      {
        nonNullable: true,
        validators: [
          Validators.required,
    ...
    length: this.#fb.control(Math.ceil(this.#data.length / 15) * 15, {
      nonNullable: true,
      validators: [
        Validators.required,
        Validators.min(15),
  ...
  readonly startTimes = Array.from(
    { length: (24 * 60) / 15 },
    (_, i) => i * 15,
  );
  readonly lengths = Array.from(
    { length: (6 * 60) / 15 - 1 },
    (_, i) => (i + 1) * 15,
  );
  readonly colors = Object.entries(appointmentColors).map(([key, value]) => ({
    key,
    value,
  }));

  readonly #matDialogRef: MatDialogRef<
    AppointmentFormDialogData,
    AppointmentFormDialogResult
  > = inject(MatDialogRef);
  ...
  static open(
    matDialog: MatDialog,
    data: AppointmentFormDialogData,
  ): Observable<AppointmentFormDialogResult> {
